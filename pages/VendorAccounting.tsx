...
import { UserProfile, Booking, Expense, ExternalInvoice, EXPENSE_CATEGORIES, Hall } from '../types';
...
import { 
  ...
  CheckCircle2, Clock, Loader2, Calendar, Search, ArrowDown, ExternalLink, Printer, User
} from 'lucide-react';
...
export const VendorAccounting: React.FC<VendorAccountingProps> = ({ user }) => {
  ...
  const fetchData = useCallback(async () => {
    try {
        const [bData, eData, iData, hData] = await Promise.all([
            ...
            supabase.from('external_invoices').select('*, halls:hall_id(name)').eq('vendor_id', user.id).order('created_at', { ascending: false }),
    ...
    externalInvoices.forEach(inv => {
        ...
        if (checkDate(date)) {
            items.push({
                ...
                type: 'income_invoice',
                date: date,
                description: `فاتورة: ${inv.customer_name}`,
  ...
  const handleSaveInvoice = async () => {
      ...
      if (!error) {
          ...
          setNewInvoice({ customer_name: '', hall_id: '', total_amount: 0, items_desc: '' });
      }
  };
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20 font-sans text-right">
      ...
      <div className="bg-white p-6 rounded-[2rem] border border-gray-200 flex flex-col lg:flex-row justify-between items-center gap-4">
         ...
         <div className="flex items-center gap-3 w-full lg:w-auto bg-gray-50 p-2 rounded-xl border border-gray-200">
            <button onClick={() => startDateRef.current?.showPicker()} className="p-2 bg-primary text-white rounded-lg"><Calendar className="w-5 h-5" /></button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
         ...
         <div className="bg-primary text-white p-6 rounded-[2rem] space-y-2 lg:col-span-1">
         ...
         <div className="bg-white p-6 rounded-[2rem] border border-gray-200 space-y-2 lg:col-span-1">
         ...
         <div className="bg-white border border-gray-200 rounded-[2rem] overflow-hidden flex flex-col min-h-[500px]">
            <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
               <div className="flex gap-2 bg-gray-50 p-1 rounded-xl w-full sm:w-auto">
                  <button onClick={() => setActiveTab('ledger')} className={`flex-1 sm:flex-none px-4 py-2 rounded-lg text-xs font-black transition-all ${activeTab === 'ledger' ? 'bg-white shadow-sm text-primary border border-gray-200' : 'text-gray-500'}`}>الكل</button>
                  <button onClick={() => setActiveTab('expenses')} className={`flex-1 sm:flex-none px-4 py-2 rounded-lg text-xs font-black transition-all ${activeTab === 'expenses' ? 'bg-white shadow-sm text-primary border border-gray-200' : 'text-gray-500'}`}>مصروفات</button>
                  <button onClick={() => setActiveTab('invoices')} className={`flex-1 sm:flex-none px-4 py-2 rounded-lg text-xs font-black transition-all ${activeTab === 'invoices' ? 'bg-white shadow-sm text-primary border border-gray-200' : 'text-gray-500'}`}>إيرادات</button>
               </div>
               <div className="relative w-full sm:w-64">
                  <input placeholder="بحث في المعاملات..." className="w-full h-10 bg-gray-50 border border-gray-200 rounded-xl px-10 text-xs font-bold focus:ring-1 ring-primary/20 outline-none" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                  ...
                  <tbody className="divide-y divide-gray-100">
                     {ledgerData
                        ...
                        >
                           ...
                           <td className="p-4">
                              <span className="bg-gray-100 text-gray-600 px-2 py-1 rounded-lg text-[10px] font-bold border border-gray-200">{item.category}</span>
                           </td>
                           <td className="p-4">
                              {item.status === 'paid' && <span className="flex items-center gap-1 text-[10px] font-bold text-green-600 bg-green-50 px-2 py-1 rounded-lg w-fit border border-green-100"><CheckCircle2 className="w-3 h-3" /> مدفوع</span>}
                              {item.status === 'pending' && <span className="flex items-center gap-1 text-[10px] font-bold text-orange-600 bg-orange-50 px-2 py-1 rounded-lg w-fit border border-orange-100"><Clock className="w-3 h-3" /> جزئي</span>}
                              {item.status === 'unpaid' && <span className="flex items-center gap-1 text-[10px] font-bold text-red-600 bg-red-50 px-2 py-1 rounded-lg w-fit border border-red-100"><Clock className="w-3 h-3" /> آجل</span>}
                           </td>
                           <td className="p-4">
          ...
          <div className="space-y-4 text-right">
              ...
              <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                      ...
                      <select className="w-full h-12 border border-gray-200 rounded-xl px-3 bg-white font-bold text-sm" value={newExpense.category} onChange={e => setNewExpense({...newExpense, category: e.target.value as any})}>
                          {EXPENSE_CATEGORIES.map(cat => <option key={cat} value={cat}>{cat}</option>)}
                      </select>
                  </div>
              </div>
              <Button onClick={handleSaveExpense} disabled={processing} className="w-full h-12 rounded-xl font-bold mt-2 shadow">
      ...
      <Modal isOpen={isInvoiceModalOpen} onClose={() => setIsInvoiceModalOpen(false)} title="إضافة إيراد خارجي" className="max-w-lg">
          <div className="space-y-4 text-right">
              <div className="space-y-2">
                  <label className="text-xs font-bold text-gray-500 flex items-center gap-1"><User className="w-3.5 h-3.5" /> اسم العميل</label>
                  <Input value={newInvoice.customer_name} onChange={e => setNewInvoice({...newInvoice, customer_name: e.target.value})} className="h-12 rounded-xl font-bold" />
              </div>
              <div className="space-y-2">
                  ...
                  <select 
                    className="w-full h-12 border border-gray-200 rounded-xl px-3 bg-white font-bold text-sm" 
              ...
              <Button onClick={handleSaveInvoice} disabled={processing} className="w-full h-12 rounded-xl font-bold mt-2 shadow">
          ...
          selectedTransaction.type === 'income_booking' ? (
              ...
              <Modal isOpen={!!selectedTransaction} onClose={() => setSelectedTransaction(null)} title="تفاصيل المعاملة" className="max-w-2xl">
                  <div className="space-y-6 text-right" id="printable-tx">
                      ...
                      <div className="bg-gray-50 p-6 rounded-[2rem] border border-gray-200 flex flex-col items-center justify-center space-y-2">
                      ...
                      <div className="flex gap-2 pt-6 border-t border-gray-100 no-print">
                          <Button variant="outline" onClick={() => window.print()} className="flex-1 rounded-xl font-bold gap-2 border-gray-200"><Printer className="w-4 h-4" /> طباعة</Button>
                          <Button 
                            variant="destructive" 
