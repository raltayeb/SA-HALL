...
export const InvoiceModal: React.FC<InvoiceModalProps> = ({ isOpen, onClose, booking }) => {
  ...
  // الضريبة محسوبة من المبلغ بعد الخصم (حسب نظام هيئة الزكاة)
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="الفاتورة الضريبية الإلكترونية" className="max-w-xl">
      <div className="space-y-6 text-right" id="printable-invoice">
        ...
        <div className="flex justify-between items-start border-b pb-6">
          <div>
            ...
            <p className="text-[10px] text-gray-400 font-bold mt-2 uppercase tracking-widest">المرفق السياحي المعتمد</p>
          ...
          <div className="text-left flex flex-col items-center gap-2">
            ...
            <span className="text-[9px] font-black text-primary bg-primary/5 px-3 py-1 rounded-full uppercase">ZATCA Verified</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-6 text-sm bg-gray-50/50 p-6 rounded-3xl border border-gray-100">
          <div className="space-y-1">
            <p className="text-gray-400 text-[10px] font-black uppercase">فاتورة إلى</p>
            <p className="font-black text-gray-900 text-lg">{booking.profiles?.full_name || booking.guest_name || 'عميل نقدي'}</p>
          ...
          <div className="text-left space-y-1 border-r border-gray-200 pr-6">
            ...
            <p className="text-gray-400 text-[10px] font-black uppercase mt-4">تاريخ الإصدار</p>
        ...
        <div className="border border-gray-100 rounded-[2rem] overflow-hidden shadow-sm">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50 bg-white">
              <tr>
                <td className="p-4">
                  <div className="font-black text-gray-900">حجز {booking.halls?.name || 'مرفق'}</div>
                  <div className="text-[10px] text-gray-400 font-bold mt-1">تاريخ المناسبة: {booking.booking_date}</div>
                </td>
                <td className="p-4 text-left font-mono font-bold text-gray-900">{formatCurrency(booking.halls?.price_per_night || 0)}</td>
              </tr>
              {booking.services && (
                <tr>
                    <td className="p-4">
                        <div className="font-black text-gray-900">{booking.services.name}</div>
                        <div className="text-[10px] text-gray-400 font-bold mt-1">خدمة إضافية</div>
                    </td>
                    <td className="p-4 text-left font-mono font-bold text-gray-900">{formatCurrency(booking.services.price)}</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="space-y-4 text-sm bg-gray-50 p-6 rounded-[2rem] border border-gray-100">
          ...
          {discountAmount > 0 && (
              <div className="flex justify-between text-red-600 font-black text-xs">
                <span className="flex items-center gap-1"><Tag className="w-3 h-3" /> الخصم المطبق ({booking.applied_coupon})</span>
          ...
          <div className="border-t border-gray-200 my-2 pt-4 flex justify-between items-center">
            <span className="font-black text-lg text-gray-900 uppercase">الإجمالي المستحق</span>
        ...
        <div className="text-center space-y-2 border-t border-gray-100 pt-6">
          <p className="text-[10px] text-gray-400 font-black uppercase tracking-[0.2em]">هذه فاتورة إلكترونية معتمدة - لا تحتاج لختم</p>
