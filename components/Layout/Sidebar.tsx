    ...
    <>
      ...
      <aside className={`fixed top-4 bottom-4 right-4 z-50 w-64 bg-card/95 backdrop-blur-md border border-border rounded-2xl flex flex-col transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-[110%] lg:translate-x-0'}`}>
        <div className="p-6 border-b border-border/50 flex flex-col items-center text-center">
          <img src="/logo.png" alt={siteName} className="w-24 h-auto mb-2" onError={(e) => (e.target as any).src = 'https://placehold.co/200x200/9b5de5/ffffff?text=LOGO'} />
          <h1 className="text-xl font-black text-primary tracking-tighter leading-tight">{siteName}</h1>
        </div>
        <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto no-scrollbar">
          {items.map((item) => (
            <button key={item.id} onClick={() => { setActiveTab(item.id); setIsOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all ${activeTab === item.id ? 'bg-primary text-primary-foreground shadow-lg shadow-primary/20 scale-[1.02]' : 'text-muted-foreground hover:bg-accent hover:text-foreground'}`}>
